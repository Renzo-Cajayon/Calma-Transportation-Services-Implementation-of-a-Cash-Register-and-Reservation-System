...
export default function AdminDashboard({ onLogout }: AdminDashboardProps) {
  ...
  return (
    <div className={`admin-shell ${sidebarOpen ? "sidebar-open" : ""}`} style={{ minHeight: '100vh', width: '100vw', backgroundColor: '#F8FAFC', overflowX: 'hidden' }}>
      
      <button className="admin-mobile-toggle" onClick={() => setSidebarOpen(!sidebarOpen)} aria-label="Toggle admin menu">
        <i className="fa fa-bars"></i>
      </button>
      {sidebarOpen && <div className="admin-sidebar-overlay" onClick={() => setSidebarOpen(false)}></div>}

      {/* --- SIDEBAR --- */}
      <aside className="admin-sidebar bg-dark text-white p-4 shadow">
        ...
        <nav className="nav flex-column gap-2">
          <Link to="/admin" onClick={() => setSidebarOpen(false)} className="nav-link text-white bg-primary rounded p-3 mb-2 shadow-sm">
            <i className="fa fa-th-large mr-2"></i> Dashboard
          </Link>

          <Link to="/admin/cars" onClick={() => setSidebarOpen(false)} className="nav-link text-white-50 p-3 mb-2 rounded hover-effect">
            <i className="fa fa-car mr-2"></i> Car Inventory
          </Link> 

          <Link to="/admin/reservations" onClick={() => setSidebarOpen(false)} className="nav-link text-white-50 p-3 mb-2 rounded hover-effect">
            <i className="fa fa-list-alt mr-2"></i> BookingList
          </Link>

          <Link to="/admin/users" onClick={() => setSidebarOpen(false)} className="nav-link text-white-50 p-3 mb-2 rounded hover-effect">
      ...
      {/* --- MAIN CONTENT AREA --- */}
      <main className="admin-main" style={{ backgroundColor: '#F8FAFC' }}>
        ...
        {overdueList.length > 0 && (
          <div className="card border-0 shadow-sm p-4 mb-4 bg-white" style={{ borderRadius: '20px', borderLeft: '8px solid #DC2626' }}>
            <div className="d-flex align-items-center justify-content-between mb-3">
              ...
              <Link to="/admin/reservations" onClick={() => setSidebarOpen(false)} className="btn btn-sm btn-outline-danger font-weight-bold px-3 rounded-pill">
