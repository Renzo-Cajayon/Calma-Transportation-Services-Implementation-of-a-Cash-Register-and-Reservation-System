...
export default function AdminReservations({ onLogout }: AdminReservationsProps) {
  ...
  return (
    <div className={`admin-shell ${sidebarOpen ? "sidebar-open" : ""}`} style={{ minHeight: '100vh', backgroundColor: '#F8FAFC', width: '100vw', overflowX: 'hidden' }}>
      ...
      <aside className="admin-sidebar bg-dark text-white p-4 shadow">
        ...
        <nav className="nav flex-column gap-2">
          <Link to="/admin" onClick={() => setSidebarOpen(false)} className="nav-link text-white-50 p-3 rounded mb-1">
          ...
          <Link to="/admin/reservations" onClick={() => setSidebarOpen(false)} className="nav-link active text-white bg-primary p-3 rounded mb-1 shadow-sm">
            <i className="fa fa-list-alt mr-2"></i> BookingList
          </Link>

          <Link to="/admin/users" onClick={() => setSidebarOpen(false)} className="nav-link text-white-50 p-3 rounded mb-1">
